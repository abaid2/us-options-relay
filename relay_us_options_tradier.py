...
#   • Tier-C sieve: nearest-weekly 0.20–0.45Δ singles if ask×100 ≤ cap (default $150) + ATM + cheap wings ($0.05–$0.40)
#   • chain_metrics.market_state = pre|regular|post|closed; per-leg mid
#   • signals/qual for direction hints; NaN/Inf → null; prev_high/prev_low robust with timesales fallback
...
def market_state_now(now: dt.datetime):
    ...
    if 13*60+30 <= hm < 20*60: return "regular"   # ~13:30–20:00 UTC ≈ 9:30–16:00 ET
...
def neighbor_weeklies(sym, event_expiry, window_days=7):
    ...
    if not exps or not event_expiry:
        return [event_expiry] if event_expiry else []
    e = dt.date.fromisoformat(event_expiry)
    prev = nextd = None
    for d in reversed(exps):
        if d < event_expiry and abs((dt.date.fromisoformat(d) - e).days) <= window_days:
            prev = d; break
    for d in exps:
        if d > event_expiry and abs((dt.date.fromisoformat(d) - e).days) <= window_days:
            nextd = d; break
...
def main():
    ...
    def process_symbol(u):
        try:
            ...
            rows = chains(u, expiry)
            if not rows:
                return {"underlier": under}

            cm = chain_metrics_from_chain(rows); cm["market_state"] = m_state
            ...
            legs=[]
            if pass_chain:
                picks = pick_targets(rows, spot, how_many=5)
            ...
            try:
                ...
                for expiry in exp_list:
                    ...
                    for tgt, rk in ((0.10,6),(0.20,4)):
                        ...
                        if p and p not in legs: legs.append(p)
                    # cheap wings
                    cheap=[o for o in rows if o.get("ask") is not None and 0.05 <= fnum(o.get("ask")) <= 0.40]
    ...
    if TIERC_ENABLE:
        ...
        for u in sorted(hv_seed):
            try:
                ...
                exps=[exp0]
                neigh = neighbor_weeklies(u, exp0, window_days=7)
                if TIERC_MAX_EXPIRIES_PER_U>1 and len(neigh)>1:
                    # include one neighbor (prefer next week if available)
                    pick = neigh[1] if neigh[0]==exp0 and len(neigh)>1 else neigh[0]
                    if pick and pick!=exp0: exps.append(pick)
                ...
                for expiry in exps:
                    ...
                    for o in picks:
                        if o not in legs: legs.append(o)
    ...
    last_good = dict(prev_last if isinstance(prev_last, dict) else {})
    quotes = []
    for m in sel_meta:
    ...
    for cm in chain_metrics:
        ...
        if key not in chains_map:
            try:
                chains_map[key] = chains(cm["u"], cm["expiry"]) or []
            except Exception:
                chains_map[key] = []
    ...
    for urow in underliers:
        u = urow["u"]
        cm = next((x for x in chain_metrics if x["u"]==u), None)
        if not cm: continue
    ...
    market_payload = {
        ...
        "state": {
            "ema": {k:v for k,v in (prev_state.get("ema") or {}).items()},
    ...
    # Optional: echo Tier-C watch into calendar to help scanners
    if GIST_ID_CAL:
        cal_payload = {
