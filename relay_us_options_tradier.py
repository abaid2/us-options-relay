...
#   - market.json   (underliers + selected option quotes + chain metrics + EMA state)
#   - history.json  (12–20 realized earnings comps per Tier-B symbol)
# Runs once per GitHub Actions invocation (cron), designed to be lightweight and resilient.
...
INTERVAL_SEC  = int(os.environ.get("INTERVAL_SEC", "300"))  # used for meta only

assert TRADIER_TOKEN and FINNHUB_TOKEN and GIST_TOKEN and GIST_ID_CAL and GIST_ID_MKT and GIST_ID_HIST, "Missing env vars."
...
def fetch_macro(from_date, to_date):
    # Free plans may 403 this endpoint; fail-open to [] so the relay keeps working.
    try:
...
def fh_earnings_history(symbol, max_quarters=20):
    """Return most-recent earnings dates (YYYY-MM-DD) up to max_quarters (newest first)."""
    dates = []
    try:
...
def quotes_options(symbols):
    ...
    while i < len(syms):
        ...
        try:
            ...
            if code in (404, 414):
                # binary split until singles; skip any singleton still failing
                stack = [chunk]
                while stack:
                    part = stack.pop()
                    if len(part) == 1:
                        try:
                            ...
                            if getattr(e2.response, "status_code", None) in (404, 414, 400):
                                continue
                            raise
                    else:
                        mid = len(part) // 2
...
def compute_comps(symbol, earn_dates, when_map=None):
    ...
    for d in earn_dates:
        w = (when_map or {}).get(d, "")
        if not w:
            w = fh_when_for_date(symbol, d) or ""
        ...
        bar_d0 = idx.get(d)
        if w == "AMC":
            if not (bar_d0 and d1 and (d in idx) and (d1 in idx)):
                continue
            c0 = idx[d]["c"]; o1 = idx[d1]["o"]; h1 = idx[d1]["h"]; l1 = idx[d1]["l"]
            gap = abs(o1 - c0)/c0
            rng = (h1 - l1)/c0
        else:  # default to BMO (or unknown)
            if not (d_1 and (d_1 in idx) and bar_d0):
                continue
...
def chain_metrics_from_chain(chain):
    ...
    Compute chain-wide:
      ...
      - chain_median_spread_pct (median of (ask-bid)/mid where bid/ask>0)
      - chain_notional_today (sum volume * mid * 100)
    Uses only fields present on /markets/options/chains rows.
    """
    oi_sum = 0
    spreads = []
    notional = 0.0
...
def main():
    ...
    # Load previous market for EMA state
    ...
    # Underliers, quotes, and chain metrics
    underliers = []
    opt_symbols = []
    sel_meta = []
    chain_metrics = []
    ...
    for u in symbols:
        try:
            ...
            # Chain metrics (no extra API calls)
            cm = chain_metrics_from_chain(chain)
            chain_metrics.append({"u": u, "expiry": expiry, **cm})

            picks = pick_targets(chain, spot, how_many=2)  # ATM + ±2 wings (keeps symbol count modest)
            if picks["atm"]:
    ...
    for cm in chain_metrics:
        ...
        ema_out[sym] = {"ema30": ema_val, "last_date": today}
